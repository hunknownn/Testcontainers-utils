...
class MultipleContainerTest {
    ...
    fun `MERGE 옵션이면 매번 같은 컨테이너로 등록되는지 테스트`() {
        ...
        val devShell2 =
            TestGenericContainer().containerShell("alpine:3.18", Recycle.MERGE, null, MasterInjectable())
    ...
    fun `NEW 옵션이면 매번 새로운 컨테이너가 등록되는지 테스트`() {
        ...
        val devShell2 = TestGenericContainer().containerShell("alpine:3.18", Recycle.NEW, null, MasterInjectable())
        val devKey2 = devShell2.key("")
